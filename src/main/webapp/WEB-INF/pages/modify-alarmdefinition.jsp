...
<script>
	...
	$(document).ready(function() {
		...
		loadComplexQuery($('#tree-container'), ${query})
...
<body>
	...
	<div>
		<form action="modify-alarmdefinition" method="POST"
			...
			<table class="table table-striped" style="text-align: center">
				...
				<tr>
					<!-- #FIXME -->
					<td>Enabled</td>
					<td><input type="radio" name="enabled" value="${enabled ? "true": "false" }" CHECKED>
						TRUE 
				...
				<tr>
					...
					<td><select name="notifications" multiple="multiple"
						style="width: 190px;" id="notificationName1">
							<c:forEach var="notificationItem" items="${notifications} "
								varStatus="status">
								<option "${notificationName.equals(notificationItem) ? "selected" : "" }">${notificationItem}</option>
							</c:forEach>
					</select></td>
				</tr>
